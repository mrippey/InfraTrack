...
class DomainSummary:
    def __init__(self, target_domain: str):
        ...
        self.api_key_header = {"x-apikey": VT_API_KEY}
    ...
    def virustotal_api_req_commfiles(self, target_domain):
        ...
        if results.get("error") and results.get("data") is None:
            ...
            if file_count == 0:
                ...
                console.print(
                    f'[!] {file_count} file(s) communicating with [bold white] {target_domain}: [bold red]{",".join(file_names)}',
                    style="bold green",
                )

                console.print(
                    f'[!] File type: [bold yellow] {",".join(file_magic)}',
                    style="bold green",
                )
    ...
    def virustotal_api_req_certinfo(self, target_domain):
        ...
        if results.get("error") and results.get("data") is None:
            ...
            if cert_count == 0:
                ...
                console.print(
                    f'[!] Serial #: [bold yellow] {",".join(cert_serial_num)}',
                    style="bold green",
                )
                console.print(
                    f'[!] Issuer CN: [bold yellow] {",".join(ca_cn)}',
                    style="bold green",
                )
                console.print(
                    f'[!] Issuer Org: [bold yellow] {",".join(ca_issuer)}',
                    style="bold green",
                )
                console.print(
                    f'[!] Subject Alternative Name: [bold yellow] {"".join(flattened_cert_san)}',
                    style="bold green",
                )
                console.print(
                    f'[!] Subject CN: [bold yellow] {"".join(subj_cn)}',
                    style="bold green",
                )
